...
export class SarsaPolicyControl<State, Action> {
  ...
  async run(): Promise<{
    ...
    const actionValueFunction = new StateActionValue<State, Action>(
      2,
      this.environment.getPossibleActions(),
      this.environment.stateToTensor.bind(this.environment)
    );
    ...
    for (let i = 0; i < this.episodes; i++) {
      ...
      while (true) {
        ...
        */
        ...
        // Update Q-values
        t0 = performance.now();
        await actionValueFunction.adjust(
          state,
          action,
          reward,
          nextState,
        ...
        // await actionValueFunction.adjust(state, action, this.alpha * tdError);
    ...
    console.log("Temporal difference SARSA - TensorFlow.js with model");
