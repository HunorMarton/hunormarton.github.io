...
In this article, I will show you how to write a plugin that extracts the first paragraph of your Markdown file and adds it to the `frontmatter` in Astro. Later, we can use it in the page metadata and when listing the posts.
...
We write a [Remark](https://github.com/remarkjs/remark) plugin that extracts the first paragraph of the Markdown file and adds it to the `frontmatter` in Astro. We use the `unist-util-visit` package to traverse the AST of the Markdown file.
